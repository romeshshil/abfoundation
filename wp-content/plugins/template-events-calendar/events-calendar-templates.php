...
if (!defined('ECT_VERSION_CURRENT')){
    define('ECT_VERSION_CURRENT', '1.2');
}
...
if (!class_exists('EventsCalendarTemplates')) {
    ...
    class EventsCalendarTemplates {
        ...
        public function __construct() {
        	...
        	*/
            ...
            add_action( 'wp_ajax_ect_hideRating',array($this,'ect_HideRating' )); 			
	...
	//	wp_enqueue_style('ect-common-styles');
		...
		$i=0;
			...
			$venue_details_html.='<div class="ect-rate-area"><span class="ect-rate-icon"><i class="fa fa-money" aria-hidden="true"></i></span>
				<span class="ect-rate">'.tribe_get_cost($event_id, true ).'</span></div>';
			endif;
			}
			...
			<span class="ev-mo">'.tribe_get_start_date($event_id, false, 'F' ).'</span></div>';
		
			}elseif($attribute['date_format']=="MD"){
			
			$event_schedule='<div class="ect-date-area '.$template.'-schedule">
			<span class="ev-day">'.tribe_get_start_date($event_id, false, 'M' ).'</span>
			<span class="ev-mo">'.tribe_get_start_date($event_id, false, 'd' ).'</span></div>';
			...
			if(!in_array($template,array("timeline","classic-timeline"))){
					$e_s_y_html='<span class="ev-yr">'.tribe_get_start_date($event_id, false, 'Y' ).'</span>';
				}
					if($template!="classic-list" && $template!="modern-list"){
			$e_s_t_h='<span class="ev-time"><small>'.$ev_time.'</small></span>';
				...
				$event_schedule='<div class="ect-date-area '.$template.'-schedule">
			<span class="ev-day">'.tribe_get_start_date($event_id, false, 'd' ).'</span>
			<span class="ev-mo">'.tribe_get_start_date($event_id, false, 'F' ).'</span> <span class="ev-yr">'.tribe_get_start_date( null, false, 'Y' ).'</span></div>';
			}
			...
			if ( tribe_get_cost() ) : 
				...
				if($template=="classic-list"){
					$event_title='<a class="ect-event-url" href="'.esc_url( tribe_get_event_link()).'" rel="bookmark"><i class="fa fa-bell"></i>'. get_the_title().'</a>';
				}
				else{
		...
		*/
		...
		}elseif($template=="modern-list"){
			include(ECT_PLUGIN_DIR.'/templates/modern-list-template.php');	
	...
	if(in_array($template,array("timeline","classic-timeline"))){
	  ...
	  function ect_Options() {
		...
		'title'=>'',
		'desc'=>'<img style="float: left;margin-right: 5px;" src="'.ECT_PLUGIN_URL.'/css/ect-icon.png">The Events Calendar Shortcode & Templates plugin provides events listing templates design and shortcode builder facility for The Events Calendar - free plugin (by Modern Tribe).',
	...
	$buynowTab= $panel->createTab( array(
	 ...
	 $extraTab->createOption( array(
		...
		.ect-classic-list .ect-classic-list-events .ect-clslist-event-details a:hover{
			background: value;
			color:#fff;
		}
		'
  ...
  .ect-classic-list-events.ect-featured-event .ect-clslist-event-date {
    background: value;
	} 
	.ect-classic-list .ect-classic-list-events.ect-featured-event .ect-clslist-event-details a:hover{
		background: value;
		color:#fff;
	}
	'
		...
		{ box-shadow: inset 0 0 8px 3px darken($event_desc_bg_color, 4%); }

		.ect-classic-list .ect-classic-list-events .ect-clslist-event-info
		{ box-shadow: inset 7px 0 10px 3px darken($event_desc_bg_color, 4%); }
	  ...
	  'css'=>'#ect-events-list-content .ect-list-post .ect-list-post-right h2.ect-list-title, #ect-events-list-content .ect-list-post .ect-list-post-right h2.ect-list-title a.ect-event-url, .cool-timeline.white-timeline .timeline-post .timeline-content h2.content-title, .cool-timeline.white-timeline .timeline-post .timeline-content h2.content-title a.ect-event-url, .ect-classic-list a.tribe-events-read-more{
	   value
	  }
	...
	/*	$panel->createOption( array(
		...
		*/
		...
		'css'=>'.ect-clslist-inner-container,
		.ect-clslist-event-price,
		...
		'css'=>'.ect-list-post .ect-list-post-right .ect-list-venue .ect-venue-details,
		...
		.ect-classic-list .ect-clslist-venue .ect-list-venue.classic-list-venue,
		.ect-classic-list .ect-list-venue .ect-google a,
		.modern-list-venue {
			value
		}
		...
		'line-height' => '0.9em',
			...
			'css'=>'.ect-list-post .ect-list-post-left .ect-list-date .ect-date-area,
			.cool-timeline.white-timeline .timeline-post .ect-date-area,
			.ect-clslist-event-date .ect-date-area,
  ...
  'css'=>'#ect-events-list-content h2.ect-events-page-title , .cool_timeline h2.ect-events-page-title, .ect-classic-list h2.ect-events-page-title{
    value
   }'
  ) );
  ...
  $stylingTab->createOption( array(
	'name'=>'Buy PRO',
	'type' => 'custom',
	'custom' => '<a href="https://1.envato.market/c/1258464/275988/4415?u=https%3A%2F%2Fcodecanyon.net%2Fitem%2Fthe-events-calendar-templates-and-shortcode-wordpress-plugin%2F20143286" target="_blank"><img style="max-width:100%;height:auto;" src="'.ECT_PLUGIN_URL.'/images/events-calendar-templates-ad.png"/></a>'
	) );
	...
	$buynowTab->createOption( array(
		...
		'type' => 'custom',
		'custom' => '<a href="https://1.envato.market/c/1258464/275988/4415?u=https%3A%2F%2Fcodecanyon.net%2Fitem%2Fthe-events-calendar-templates-and-shortcode-wordpress-plugin%2F20143286" target="_blank"><img style="max-width:100%;height:auto;" src="'.ECT_PLUGIN_URL.'/images/ect-buy-pro.png"/></a>'
		) );
...
public function ect_tribe_event_recurringinfo( $before = '', $after = '', $link_all = true ) {
   ...
   public function ect_admin_messages() {
    ...
    jQuery(\'.coolHideRating\').click(function(){
        var data={\'action\':\'ect_hideRating\'}
             jQuery.ajax({
